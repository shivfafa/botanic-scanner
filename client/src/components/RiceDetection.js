...
export default function RiceDetection() {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/predict/rice", {
        method: "POST",
        body: formData,
      });
  ...
  const title = "Rice Disease Detection";
  const riceimage=rice;
  const color ="black";
  ...
  return (
    <>
      <MainImageSection title={title} image={riceimage}  color={color}/>

      <Wrapper>
        <h1>Rice Crop Disease Detection</h1>
        <UploadContainer>
          <UploadArea
        ...
        {prediction && (
          ...
          <NavLink to={getPrecautionPath(prediction.class)}><button className="btn">Precautions</button></NavLink>
      ...
      {showAlert && (
        <DialogFlow
          ...
          message1="1. Upload/Choose a Clear Image of Rice Leaf"
