...
import wheat from "../images/wheat2.jpg";
...
export default function WheatDetection() {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/predict/wheat", {
        method: "POST",
        body: formData,
      });
  ...
  const title = "Wheat Disease Detection";
  const wheatimage = wheat;
  const color = "white";
  ...
  return (
    <>
      <MainImageSection title={title} image={wheatimage} color={color} />

      <Wrapper>
        <h1>Wheat Crop Disease Detection</h1>
        <UploadContainer>
          <UploadArea
      ...
      {showAlert && (
        <DialogFlow
          ...
          message1="1. Upload/Choose a Clear Image of Wheat Leaf"
