...
import sugarcane from "../images/sugarcane2.jpg";
...
export default function SugarDetection() {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/predict/sugarcane", {
        method: "POST",
        body: formData,
      });
  ...
  const title = "Sugarcane Disease Detection";
  const sugarcaneimage = sugarcane;
  const color = "white";
  ...
  const getPrecautionPath = (predictionClass) => {
    switch (predictionClass) {
      case "Sugarcane_Bacterial_Blight":
        return "/sbacterialblight";
      case "Sugarcane_Red_Rot":
        return "/redrot";
      case "Sugarcane_Red_Rust":
        return "/redrust";
      default:
        return "/sugarcanehealth";
    }
  };

  return (
    <>
      <MainImageSection title={title} image={sugarcaneimage} color={color} />

      <Wrapper>
        <h1>Sugarcane Crop Disease Detection</h1>
        <UploadContainer>
          <UploadArea
        ...
        {prediction && (
          <Predict>
            ...
            <NavLink to={getPrecautionPath(prediction.class)}>
              <button className="btn">Precautions & Treatment</button>
            </NavLink>
          </Predict>
        )}
      </Wrapper>
      {showAlert && (
        <DialogFlow
          ...
          message1="1. Upload/Choose a Clear Image of Sugarcane Leaf"
