...
import cotton from "../images/cotton2.jpg";
...
export default function CottonDetection() {
  ...
  const handleFileUpload = async (file) => {
    ...
    try {
      const response = await fetch("http://localhost:8000/predict/cotton", {
        method: "POST",
        body: formData,
      });
  ...
  const title = "Cotton Disease Detection";
  const cottonimage = cotton;
  const color = "black";
  ...
  return (
    <>
      <MainImageSection title={title} image={cottonimage} color={color} />

      <Wrapper>
        <h1>Cotton Crop Disease Detection</h1>
        <UploadContainer>
          <UploadArea
        ...
        {prediction && (
          <Predict>
            ...
            <NavLink to={getPrecautionPath(prediction.class)}>
              <button className="btn">Precautions</button>
            </NavLink>
          </Predict>
        )}
      </Wrapper>
      ...
      {showAlert && (
        <DialogFlow
          ...
          message1="1. Upload/Choose a Clear Image of Cotton Leaf"
